...
token = "7670310044:AAGRpEMo3q5EHEBpybebnW5XjqyRpiYbKtc"
bot = Bot(token=token)
...
async def set_inline_menu(message):
    await message.answer('Выберите опцию:', reply_markup=inline_kbd)

@dp.callback_query_handler(text= 'formulas')
async def get_formulas(call):
    await call.message.answer('10 x вес (кг) + 6,25 x рост (см) – 5 x возраст (г) – 161) x')
    await call.answer()
...
async def info(message):
    await message.answer(
        ...
        ' упрощённой формуле Миффлина - Сан Жеора для женщин ')
